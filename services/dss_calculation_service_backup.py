...
def _build_explanation(
    ...
    if not result["admissible"]:
        ...
        if result["threshold_reasons"]:
            reasons.append(
                "Альтернативу виключено через порогові обмеження."
            )
            reasons.extend(result["threshold_reasons"])

        rejected_rules = [
            event["message"]
            for event in result["rules_triggered"]
            if event["action_type"] == "reject"
        ]

        if rejected_rules:
            reasons.append(
                "Альтернативу виключено правилом IF–THEN."
            )
            reasons.extend(rejected_rules)

        return reasons or ["Альтернативу виключено."]
    ...
    for event in result["rules_triggered"]:
        if event["action_type"] in {"penalty", "bonus"}:
...
def calculate_ranking(
    ...
    *,
    ...
    for row in rows:
        threshold_admissible, threshold_reasons = _check_thresholds(
            row,
            criteria,
        )
        ...
        rule_evaluation = evaluate_rules_for_values(
            row["raw_values"],
            row["alternative"],
        )
        admissible = (
            threshold_admissible
            and not rule_evaluation["rejected"]
        )
        adjusted_score = apply_rule_adjustments(
            integral_score,
            rule_evaluation,
        )
        ...
        result = {
            ...
            "base_score": integral_score,
            "score": adjusted_score if admissible else 0.0,
            "rule_score_factor": rule_evaluation["score_factor"],
            "rules_triggered": rule_evaluation["events"],
